...
async function loadTodos() {
  ...
  try {
    ...
    snapshot.forEach((docSnap) => {
      const todo = { id: docSnap.id, ...docSnap.data() } as TodoItem;
...
function showAddTodoModal() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    const title = (document.getElementById('todoTitle') as HTMLInputElement).value;
    const description = (document.getElementById('todoDescription') as HTMLTextAreaElement).value;
    const priority = (document.getElementById('todoPriority') as HTMLSelectElement).value as 'high' | 'medium' | 'low';
