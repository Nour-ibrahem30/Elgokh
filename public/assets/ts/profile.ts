...
function showConfirmDialog(message: string, onConfirm: () => void, onCancel?: () => void) {
...
// Enhanced Authentication with Complete Backend Integration
    ...
    onAuthStateChanged(auth, async (user) => {
      if (!user) {
        ...
        if (storedEmail && storedUser) {
          try {
            ...
            // Get the actual login name from localStorage or extract from email
            ...
            // Update UI immediately with the correct name
      ...
      try {
        ...
        if (!userDoc.exists()) {
          ...
          // Redirect teachers to dashboard (keep original behavior)
        ...
        // Update UI elements immediately with correct name
        ...
        if ((window as any).updateNavUserInfo) {
          (window as any).updateNavUserInfo({ name: displayName, email: userEmail });
        }
        
        // Make user available globally
        (window as any).currentUser = { name: displayName, email: userEmail, uid: currentUser.uid };
...
function updateProfileUI(displayName: string, userEmail: string) {
  ...
  // Function to actually update the elements
  const doUpdate = () => {
    ...
    if (userNameEl) {
      ...
      userNameEl.style.color = '#f1f5f9'; // Ensure visibility
    ...
    if (userEmailEl) {
      ...
      userEmailEl.style.color = '#94a3b8'; // Ensure visibility
    ...
    if (userInitialsEl) {
      ...
      userInitialsEl.style.color = '#f1f5f9'; // Ensure visibility
...
async function loadProgress() {
  ...
  try {
    ...
    const videoBar = document.getElementById('videoProgressBar') as HTMLElement;
    const examBar = document.getElementById('examProgressBar') as HTMLElement;
...
function shouldShowTodo(todo: TodoItem): boolean {
...
function createTodoItem(todo: TodoItem & { id: string }): HTMLElement {
  ...
  const deleteBtn = div.querySelector('.delete-todo') as HTMLButtonElement;
...
function showAddTodoModal() {
  ...
  const form = modal.querySelector('#addTodoForm') as HTMLFormElement;
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      hideLoading();
      showToast('حدث خطأ أثناء إضافة المهمة: ' + (error as Error).message, 'error');
    }
  });
...
function showSampleExamResults(container: HTMLElement) {
  const sampleResults = [
...
function switchTab(tabName: string) {
  ...
  // Re-query tab buttons and content to ensure they exist
...
function initializeEventListeners() {
  ...
  setTimeout(() => {
    ...
    document.addEventListener('click', (e) => {
      if (navMenu && !navMenu.contains(e.target as Node) && !navToggle?.contains(e.target as Node)) {
...
function startMotivationalMessages() {
  ...
  // Show motivational message every 30 minutes (1800000 ms)
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (currentUser) {
      ...
      // Force update profile UI with current user data
      ...
      // Update UI multiple times to ensure it works
      updateProfileUI(displayName, userEmail);
      ...
      // Initialize event listeners after everything is ready
      console.log('🔧 Initializing event listeners...');
      initializeEventListeners();
      
      // Start with todos tab
