...
async function loadTodosFromFirebase() {
  if (!currentUser || !db) {
    console.log('⚠️ Cannot load todos: user or db not ready');
    return;
  }
    ...
    videos.slice(0, 5).forEach(video => {
      ...
      if (!existingTodo) {
        todos.push({
          ...
          title: `شاهد: ${video.title}`,
          description: video.description || 'فيديو تعليمي جديد',
          completed: false,
          priority: 'medium',
          type: 'video',
          contentId: video.id,
    ...
    exams.slice(0, 5).forEach(exam => {
      ...
      if (!existingTodo) {
        todos.push({
          ...
          description: exam.description || 'امتحان جديد متاح',
          completed: false,
          priority: 'high',
          type: 'exam',
          contentId: exam.id,
    ...
    materials.slice(0, 3).forEach(material => {
      ...
      if (!existingTodo) {
        todos.push({
          ...
          title: `راجع: ${material.title}`,
          description: material.description || 'مادة تعليمية جديدة',
          completed: false,
          priority: 'low',
          type: 'material',
    ...
    notes.slice(0, 3).forEach(note => {
      ...
      if (!existingTodo) {
        todos.push({
          ...
          title: `اقرأ: ${note.title}`,
          description: note.content ? note.content.substring(0, 100) : 'ملاحظة جديدة',
          completed: false,
          priority: 'low',
          type: 'note',
          contentId: note.id,
...
function loadTodos() {
  ...
  if (filteredTodos.length === 0) {
    todoList.innerHTML = `
      <div style="text-align: center; padding: 3rem; color: #94a3b8;">
        ...
        <p>${filter === 'completed' ? 'لا توجد مهام مكتملة' : 'لا توجد مهام'}</p>
      </div>
    `;
    return;
  }
  ...
  filteredTodos.forEach(todo => {
    ...
    const typeIcons = { video: '🎥', exam: '📝', material: '📚', note: '📋', manual: '✏️' };
    const typeIcon = typeIcons[todo.type] || '✏️';
    
    // Create action button based on type
    let actionButton = '';
    if (todo.type === 'video' && !todo.completed) {
      actionButton = `<button class="btn-action" data-action="watch" data-id="${todo.contentId}">شاهد الآن</button>`;
    } else if (todo.type === 'exam' && !todo.completed) {
      actionButton = `<button class="btn-action" data-action="solve" data-id="${todo.contentId}">حل الامتحان</button>`;
    } else if (todo.type === 'material' && !todo.completed) {
      actionButton = `<button class="btn-action" data-action="view" data-id="${todo.contentId}">عرض المادة</button>`;
    } else if (todo.type === 'note' && !todo.completed) {
      actionButton = `<button class="btn-action" data-action="read" data-id="${todo.contentId}">اقرأ الآن</button>`;
    }
    ...
    todoElement.innerHTML = `
      ...
      <div class="todo-content">
        ...
        <div class="todo-meta">
          ...
          ${todo.autoGenerated ? '<span class="todo-badge">تلقائي</span>' : ''}
        </div>
        ${actionButton}
      </div>
      <div class="todo-actions">
        ${!todo.autoGenerated ? `<button class="btn-icon delete-todo" data-id="${todo.id}" title="حذف المهمة">🗑️</button>` : ''}
      </div>
    `;
